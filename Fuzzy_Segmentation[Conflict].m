...
try
  ...
  %  I = imfilter(double(I), hg, 'replicate');
    gradI = calc_Grad_I(I);
    invG = max(1./(1+(gradI./(std(gradI(:)))).^q),0.01);
    ...
    while err>=min_err&&itr<max_itr
        ...
        if itr==1
            ...
            mu_cropped = cellfun(@FindMostLiklymuGray,nBG_cropped,invG_cropped,mu_cropped,P_pred,'uniformoutput',0);
           ...
           % Speed_cropped = cellfun(@fixSpeedIm,Speed_cropped,mu_cropped,P_pred,'uniformoutput',0);
            ...
            mu_cropped = cellfun(@FindMuCOM,U_cropped,'uniformoutput',0);
            if save_debug
                DEBUG{itr}.mu_cropped = mu_cropped;
            end
            Speed_cropped = cellfun(@(nbg,invg) max((nbg).*(invg),1e-8),nBG_cropped,invG_cropped,'uniformoutput',0);
    ...
    for l = unique(L(L>0))'
        ...
        removeCandidate = NotBG_Candidates_l(([NotBG_Candidates_l.Solidity]<=Solidity_Thr./2|[NotBG_Candidates_l.Area]<=min_cell_size));;
        ...
        for i = 1:length(NotBG_Candidates_l)
            ...
            if any(idxx==1)||any(idxy==1)||any(idxx==Width-1)||any(idxy==Height-1)%any(idxx==11)||any(idxy==11)||any(idxx==Width-10)||any(idxy==Height-10)
                flags(i) = true;
...
function mu = FindMostLiklymuGray(PG,invG,mu,sigma)
[X,Y] = meshgrid(1:size(PG,2),1:size(PG,1));
G = reshape(mvnpdf([Y(:),X(:)],mu',rot90(sigma(1:2,1:2),2)),size(X));

[muy,mux]=find((PG.*invG.*G)==max(PG(:).*invG(:).*G(:)),1);
%mu = round([Y(:),X(:)]'*(PG(:).*Phi(:))./sum(PG(:).*Phi(:)));
mu = [muy;mux];
